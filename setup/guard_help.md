...
- Отключение голосования на основном сервере производится в следующей последовательности:
	- Смена ключа валидатора на неголосующий на основном сервере.
	- При удачной смене ключа на основном сервере, с него копируется тауэр и на нем отключается сервис 'telegraf'.  
	- При неудачной попытке замены ключа на основном сервере, на нем производится перезапуск сервиса соланы в режиме "NoVoting".
	- Если перезапуск сервиса так же не удался, 
