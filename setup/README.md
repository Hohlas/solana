...
## Server setup
```bash
sudo apt update && sudo apt upgrade -y && sudo apt install sysstat git ncdu ufw tmux htop atop curl nano fail2ban smartmontools mc man rsync cron logrotate rsyslog encfs jq zip unzip -y
```
