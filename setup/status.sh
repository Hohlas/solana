...
if [ $(grep -c $voteAccount <<< $validatorCheck) == 0  ]; then echo "validator not found in set"; exit 1; fi
 ...
 echo '+++'
 echo "pubkey=$identityPubkey ver=$version"
case $status in #status 0=validating 1=up 2=error 3=delinquent 4=stopped
  0) echo "status: validating";;
  1) echo "status: up";;
  2) echo "status: error";;
  3) echo "status: delinquent";;
  4) echo "status: stopped";;
