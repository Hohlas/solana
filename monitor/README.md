...
replay_to_vote_time - ее нет в логах, вычисляется как разность времен появления метрик 'tower-vote latest' - 'replay-slot-stats' для того же слота.. Показывает, насколько быстро нода может голосовать после получения и обработки слота.

fork_to_replay_time - тоже нет в логах. Время между обнаружением нового форка (new fork) и завершением обработки (replay-slot-stats). Увеличение может указывать на проблемы с производительностью ноды.
